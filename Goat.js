...
const watchAndReloadConfig = (dir, type, prop, logName) => {
	...
	fs.watch(dir, (eventType) => {
		if (eventType === type) {
			...
			setTimeout(() => {
				try {
					// if file change first time (when start bot, maybe you know it's called when start bot, maybe you know it's called when start bot?) => not reload
					if (isFirstModified) {
...
(async () => {
	...
	// Add ping endpoint for uptime monitoring
	const express = require("express");
	const app = express();

	// Add ping endpoint for uptime monitoring
	const pingRouter = require('./ping-endpoint.js');
	app.use('/', pingRouter);
